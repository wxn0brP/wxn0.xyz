...
export function startShop(args: string = "") {
	if ($store.level.get() < 5) {
		print(
			"Access Denied: Dark Market requires Level 5 security clearance.",
			"error",
		);
		return;
	}
	...
	if (command[0] === "buy") {
		...
		if (credits < item.cost) {
			print(
				`Insufficient funds. You have <span class="warning">${credits}</span> credits, but item costs <span class="error">${item.cost}</span>.`,
				"error",
			);
			return;
		}
	...
	print(
		`Credits Available: <span class="warning">${$store.credits.get()}</span>`,
	);
	print(
		`Current XP Multiplier: <span class="success">${$store.xpMultiplier.get()}x</span><br>`,
	);
