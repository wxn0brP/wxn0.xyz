...
export function showStatus() {
	...
	print(
		`XP: <span class="success">${$store.xp.get()}/${getXpToNextLevel($store.level.get())}</span>`,
	);
	print(`Credits: <span class="warning">${$store.credits.get()}</span>`);
	print(
		`XP Multiplier: <span class="dim">${$store.xpMultiplier.get()}x</span>`,
	);
}
...
export function showLinks() {
	...
	if (unlockedLinks.length === 0) {
		...
		unlockedLinks.forEach(l => {
			print(
				`  <a href="${l.url}" target="_blank">${l.name}</a> - Level ${l.level}`,
			);
		});
	}
}
