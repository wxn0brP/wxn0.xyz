...
export const hackingMission: HackingMission = {
	active: false,
	target: null,
	vulnerability: null,
	command: null,
...
function failHack() {
	...
	if (hackingMission.timer) clearTimeout(hackingMission.timer);

	hackingMission.active = false;
}

export function tryHack(input: string) {
	if (!hackingMission.active) return;
	...
	if (input.toLowerCase() === hackingMission.command) {
		...
		print(
			`Hacking... success! Gained <span class="success">${xpGained}</span> XP, <span class="warning">${creditsGained}</span> Credits.`,
		);
		...
		print(
			`Incorrect command. Expected '<span class="system">${hackingMission.command}</span>'.`,
			"error",
		);
		failHack();
	}
...
export async function startHack() {
	if (hackingMission.active) {
		print(
			"A hack is already in progress. Complete it or let it time out.",
			"system",
		);
		return;
	}
	...
	print(
		`Found vulnerability: <span class="dim">${vulnerability.name}</span>`,
		"system",
	);
	print(
		`To exploit, type: <span class="success">${vulnerability.command}</span>`,
	);
