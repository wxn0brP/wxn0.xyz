...
export function cmdMail(args: string[], fullArgs: string) {
	...
	if (args.length === 0) {
		...
		mails.forEach((mail, index) => {
			...
			print(
				`[${index + 1}] ${status} ${mail.from}: ${mail.subject} <span class="dim">(${date})</span>`,
			);
		});

		print("<br>", "dim").textContent +=
			"Usage: mail read <number> | mail send <message>";
		return;
	}
	...
	if (args[0] === "send") {
		...
		const mailURL = mailCodes
			.map(code =>
				typeof code === "string" ? code : String.fromCharCode(code),
			)
			.join("");
