...
export async function cmdNews() {
	...
	try {
		const response = await fetch(
			"https://api.github.com/repos/wxn0brP/wxn0.xyz/commits?per_page=100",
		);
		if (!response.ok) {
		...
		for (const commit of commits) {
			...
			print(
				`<span style="color: #aaa">[${commit.commit.author.date.substring(0, 10)}]</span> <span style="color: #fff">${message}</span>`,
			);
			count++;
